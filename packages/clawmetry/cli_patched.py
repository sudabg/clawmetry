...
def _cmd_connect(args) -> None:
    ...
    try:
        result = validate_key(api_key, hostname=machine_hostname, existing_node_id=_saved_node_id)
        node_id = result.get("node_id") or _saved_node_id or machine_hostname
        print(f"✅")
    ...
    if _saved_node_id and _saved_node_id == node_id:
        print(f"  (reused existing node registration)")
    print()
    if is_new_key:
        print("  Keep this encryption key safe (like a password):")
        print(f"  {enc_key}")
    else:
        print("  Encryption key: (preserved from previous config)")
    print()
